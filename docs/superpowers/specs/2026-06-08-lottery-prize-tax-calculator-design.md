...
## 13. 省税拆票(2026-06-08,已与用户确认)

利用"每张彩票单独计税"规则,把一张会过线的高倍票拆成多张都不过线的票,合法避税。
- 入口:新增「拆票」tab。输入:彩种 + 玩法 + 想买总倍数 + **实名张数上限**(0=全免实名)。
- **拆分依据 = 该玩法最高奖等**(确保任何中奖结果都不过线;快乐8 多档位按顶档算)。
- 算法:`taxFreeCap=⌊10000/P⌋`(每票免税最大倍数)、`noNameCap=⌊3000/P⌋`(每票免实名最大倍数)。
  在实名额度内用满 `taxFreeCap` 打包减少张数,剩余装进 `noNameCap` 的免实名票;`P>10000` 时不可避税(提示),`P>3000` 时实名不可避(每票均实名)。
- 输出:拆分方案(各票倍数/最高奖金/是否实名)、合计奖金(不变)、合计税(0)、实名张数,及与"不拆分"对比的省税额。
- 引擎纯函数 `splitTicket`(`src/engine/split.ts`),TDD 覆盖:免税拆分、0实名拆分、单注超1万不可避、单注超3千实名不可避。
