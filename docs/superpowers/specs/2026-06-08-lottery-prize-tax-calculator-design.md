...
### 13.1 组合 + 规则(2026-06-08 增补,已确认)
- **组合选择**:数字彩可输入多玩法组合(直选+组三+组六);快乐8 单玩法。引擎 `splitCombination`。
- **两种模式**:`separate`(各玩法分开拆,每票单玩法)/ `mixed`(混合打包,一票多玩法,贪心装箱)。
- **规则感知**:拆分时套用启用的活动规则,且**门槛按每张小票各自判定**——每票每玩法的倍数上限按"规则后实际中奖额 ≤ 阈值"动态求(`maxMultUnderCap` 扫描,奖金随倍数单调不减)。
- **到手对比**:展示"不拆分(整票享加奖但缴税)到手"与"拆分(免税但可能丢加奖)到手";当不拆分到手更多时如实提示(典型:加奖门槛"满N元"导致达标即过税线)。
- TDD 覆盖:单/多玩法 separate、mixed 更少张、规则致"拆分丢加奖"的到手对比。

## 14. 号码输入 + 快乐8 多注中奖可能性分析(2026-06-10,已与用户确认)

承接 §12.1 的"组合票"思路,本期解决两件事:① 快乐8 一张票多注、不同注可同时中不同奖等,合计可能跨税/实名门槛;② 用户输入号码后,**由 app 反向分析"这张票存不存在需要缴税/实名的中奖可能"**,而非由用户声明中了几等。

**已确认的范围裁剪**
- **不做扫码**:彩票票面二维码记录的是兑奖用的票号/序列号,非公开可解析的"号码+玩法"格式,第三方扫了也还原不出投注内容;且手机摄像头(getUserMedia)需 HTTPS 安全上下文,当前纯 http(IP 直连)调不起。故本期只做手动输入,扫码待有域名+HTTPS 后再评估。
- **不做开奖兑奖**:不输入开奖号码、不判定"实际中没中";只做"可能性"分析。

### 14.1 快乐8「多注票 + 可能性分析」(替代原 calc 页单玩法视图)
- 一张票 = 多条注,每条注 = **玩法(选一~选十) + 倍数**,可选展开输入真实号码。**默认不选"中几个"**。
- **顶部结论(主输出)**:
  - 最高可中 = Σ(每注顶档奖金 × 倍数)。
  - `> 10000` → "存在需要**缴税**的中奖情况";`> 3000` → "存在需要**实名**的中奖情况";都不超 → "✅ 任何中奖都无需缴税/实名"。
  - 含浮动头奖(选九中九/选十中十,prize=null)→ "含浮动头奖,合计无法精确,但**必然需缴税+实名**"。
- **逐注明细**:每注列出各档位金额(×倍数),逐档标"实名/缴税",展示风险来源(保留旧 ResultList 的"看全玩法奖金表"价值)。
- **(可选)锁定具体场景**:可为每注手动锁定"中几个",app 给出精确合计、税后与税/实名判定——满足"一注中一等奖、一注中二等奖"求和过线的精确建模。

### 14.2 手动输入号码(可输可不输)
- **福彩3D / 排列3**:输 3 个数字 → 按重复模式**自动锁定可选玩法**:全同(`111`)仅直选;恰一对(`112`)直选+组三(禁组六);全不同(`123`)直选+组六(禁组三)。从输入端根治 §"组三/组六同时勾选"问题。
- **快乐8**:输入所选号(1–80,去重、范围、个数 1–10 校验)→ 个数**自动定玩法(选N)**;锁定场景时"中几个" ≤ 所选个数。
- 不输号码:沿用下拉选玩法,行为不变。

### 14.3 引擎与组件
- 引擎抽出统一的 `computeTicket(gameId, entries, rules, nowMs)`:
  - `TicketEntry { playId; multiplier; lockedTierId?: string; numbers?: number[] }`。
  - 数字彩:沿用每玩法单档 + 组三/组六"取较高者不相加"互斥(§12.1)。
  - 快乐8:`lockedTierId` 缺省时按**顶档**参与"最高可中";设置时按该档精确求和。多注之间相加(无互斥)。
  - 返回在 `TicketResult` 基础上增加 `maxTotal`、`floating`(是否含浮动头奖)、`existsTax`/`existsRealname`(可能性结论)。
...
### 14.5 号码输入约束 + 倍数规则(2026-06-10,已确认)
- **号码输入**:① 仅允许数字(非数字一律剔除);② 快乐8 每个号 **2 位**(个位补 0,如 3→`03`),连续输入**每 2 位自动切分换位**(`parseKl8Pairs`/`formatKl8Pairs`),范围 1–80、不重复、1–10 个;③ 3D/排列3 每位 **1 位**数字、共 3 位,无需手动空格/逗号。
- **倍数上限**:**福彩(3D、快乐8)最多 15 倍**(`MultiplierStepper` 增 `max`);体彩排列3 沿用默认。
- **快乐8 倍数全票一致**:倍数移到**整票级**(`Kl8Ticket.multiplier`),各注不再单独设倍;`computeKl8Ticket` 各注按整票倍数计算。
